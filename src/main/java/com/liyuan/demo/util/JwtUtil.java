...
public class JwtUtil {
     ...
     * @return
     */
    public static JwtUser checkLogin(String authorization) throws MongoSpringBootException {
        JwtUser jwtUser;

        Claims claims = getClaims(authorization);
        if (claims == null) {
            throw new MongoSpringBootException(202,"无效token");
        }

        String usercode = (String) claims.get("usercode");
        if(usercode == null || usercode.equals("")){
            throw new MongoSpringBootException("会话中的用户编号为空");
        }

        Long expiration = (Long) claims.get("expiration");
        if(expiration < (new Date().getTime())){
            throw new MongoSpringBootException(202,"会话超时，请重新登陆");
        }

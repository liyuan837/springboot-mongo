...
public abstract class BaseServiceImpl<T, C extends BaseCondition, M extends BaseMapper<T, C>> implements BaseService<T, C> {
    ...
    @Override
    public T queryWithValid(Object id) throws MongoSpringBootException {
        T o = mapper.select(id);
        if (o == null) {
            throw new MongoSpringBootException(id + "对应的记录为空");
        }
        return o;
    }
    ...
    @Override
    @Transactional(rollbackFor = {Exception.class})
    public void updateWithValid(T po) throws MongoSpringBootException {
        int row = mapper.update(po);
        if (row == 0) {
            throw new MongoSpringBootException("修改失败");
        }
    }
